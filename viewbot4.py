...
# rotate Tor identity every N requests (higher to reuse circuits)
ROTATE_FREQ          = 20
...
def wait_for_bootstrap(ctl, timeout=60):
    deadline = time.time() + timeout
    while time.time() < deadline:
        status = ctl.get_info('status/bootstrap-phase')
        if 'PROGRESS=100' in status:
            return
        time.sleep(0.5)
    raise RuntimeError('Tor failed to bootstrap in time')


def renew_tor_identity():
    with Controller.from_port(port=TOR_CONTROL_PORT) as ctl:
        ctl.authenticate()
        wait_for_bootstrap(ctl)
